...
// 16 DN Sequences as per your instructions
const DN_SEQUENCES = [
...
// Logic B: Direction Determination
...
// Logic A: Find Station Area (Outer Home Signals)
function getStationArea(stnName) {
    ...
    if(homes.length > 0) {
        ...
        return { lat: (minLat + maxLat)/2, lng: (minLng + maxLng)/2, radius: (maxLng - minLng) * 55000 + 500 }; 
    }
    // Fallback to station.csv
...
window.onload = function() {
    Papa.parse("station.csv", {download:true, header:true, complete: r => {
    ...
    const sigFiles = [{f:'up_signals.csv', t:'UP', c:'green'}, {f:'dn_signals.csv', t:'DN', c:'blue'}, {f:'up_mid_signals.csv', t:'UP_MID', c:'red'}, {f:'dn_mid_signals.csv', t:'DN_MID', c:'purple'}];
...
function generateLiveMap() {
    ...
    Papa.parse(f, {header:true, skipEmptyLines:true, complete: function(res) {
        ...
        // Logic C: Slice Data
        let startIndex = rawData.findIndex(p => Math.sqrt(Math.pow(p.lt-areaF.lat,2)+Math.pow(p.lg-areaF.lng,2)) < 0.008);
        let endIndex = rawData.findIndex((p, i) => i > startIndex && Math.sqrt(Math.pow(p.lt-areaT.lat,2)+Math.pow(p.lg-areaT.lng,2)) < 0.008);
        window.rtis = (startIndex !== -1 && endIndex !== -1) ? rawData.slice(startIndex, endIndex + 1) : rawData;

        // Logic A: Draw Orange Zones
        [stnF, stnT].forEach(name => {
            let a = getStationArea(name);
            L.circle([a.lat, a.lng], {radius: a.radius || 500, color: 'orange', fillOpacity: 0.3, weight: 1}).addTo(map);
            L.marker([a.lat, a.lng], {icon: L.divIcon({className:'stn-label', html: name + " STN"})}).addTo(map);
        });

        // Logic D: Custom Signal Icons & Plotting
        window.master.sigs.forEach(sig => {
            ...
            if(match.length > 0) {
                ...
                let spd = match[0].spd;
                let icon = L.divIcon({html: `<div style="background:${sig.clr}; width:12px; height:12px; border-radius:50%; border:2px solid white;"></div>`});
        ...
        L.polyline(path, {color: '#333', weight: 3}).addTo(map);
        map.fitBounds(path);
        document.getElementById('log').innerText = `Mapped: ${stnF} to ${stnT} (${dir})`;
    }});
}
