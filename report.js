window.saveInteractiveWebReport = function() {
    ...
    body{margin:0;display:flex;height:100vh;background:#eee;font-family:sans-serif;}
    #side{width:350px;background:#fff;color:#333;padding:15px;overflow-y:auto;border-right:1px solid #ccc;}
    #map{flex:1;} .card{background:#f9f9f9;padding:10px;margin-bottom:8px;border-radius:4px;cursor:pointer;border:1px solid #ddd;border-left:6px solid;}
    </style></head><body><div id="side"><h3>SECR RAIPUR REPORT</h3><b>${sF} to ${sT}</b><hr>${sigData.map(r=>`<div class="card" style="border-left-color:${r.clr}" onclick="m.setView([${r.lt},${r.lg}],17)"><b>${r.n}</b><br>Speed: ${r.s} | Time: ${r.t}</div>`).join('')}</div>
    <div id="map"></div><script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script><script>
    var m=L.map('map').setView([${sigData[0].lt},${sigData[0].lg}],14);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(m);
    var pathData = ${JSON.stringify(window.rtis.map(p=>({lt:p.lt, lg:p.lg, s:p.spd, t:p.time})))};
    var poly=L.polyline(pathData.map(p=>[p.lt,p.lg]),{color:'black',weight:4}).addTo(m);
    m.fitBounds(poly.getBounds());
    
    poly.on('mousemove', function(e) {
        let pt = pathData.reduce((a, b) => Math.abs(b.lt-e.latlng.lat) < Math.abs(a.lt-e.latlng.lat) ? b : a);
        L.popup().setLatLng(e.latlng).setContent("Speed: "+pt.s+" Kmph<br>Time: "+pt.t).openOn(m);
    });

    ${JSON.stringify(stnData)}.forEach(a => {
        L.marker([a.lt, a.lg], {icon:L.divIcon({html:'<b style="font-size:12px;white-space:nowrap">'+a.n+'</b>', className:''})}).addTo(m);
    });

    ${JSON.stringify(sigData)}.forEach(s => {
        L.circleMarker([s.lt,s.lg], {radius:7, color:s.clr, fillColor:'white', fillOpacity:1, weight:2}).addTo(m).bindPopup(s.n+"<br>Spd: "+s.s+"<br>Time: "+s.t);
    });
    </script></body></html>`;
